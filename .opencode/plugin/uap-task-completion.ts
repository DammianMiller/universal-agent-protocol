...
export const UAPTaskCompletion: Plugin = async ({ $, directory }) => {
  ...
  return {
    ...
    // On compaction, save session task summary and show session dashboard
    'experimental.session.compacting': async (_input, output) => {
      ...
      // Show compact session dashboard on compaction
      try {
        await $`node ${projectDir}/dist/bin/cli.js dash session --compact`.quiet();
      } catch {
        /* ignore if CLI not built */
      }
    },
  };
};
