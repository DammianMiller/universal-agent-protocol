...
export const UAMPatternRAG: Plugin = async ({ client, $ }) => {
  ...
  return {
    event: async ({ event }) => {
      if (event.type === "session.created") {
        try {
          ...
          const result = await $`${VENV_PYTHON} ${QUERY_SCRIPT} "coding agent best practices security" --top 2 --format context`.quiet()
          const patterns = result.stdout.toString().trim()
          if (patterns) {
            console.log("[UAM-RAG] Loaded initial patterns")
          }
        } catch {
          // Qdrant may not be running — fail silently
          console.log("[UAM-RAG] Pattern loading skipped (Qdrant unavailable)")
        }
      }
    },
    ...
    "experimental.session.compacting": async (_input, output) => {
      try {
        ...
        if (patternList) {
          ...
          output.context.push(
            `<uam-patterns-memo>Previously active patterns: ${patternList}. Re-query if needed.</uam-patterns-memo>`
          )
        }
      } catch { /* fail safely */ }
    },
    ...
    middleware: async (input, next) => {
      ...
      try {
        ...
        const taskText = typeof lastMessage.content === "string"
          ? lastMessage.content
        ...
        const result = await $`${VENV_PYTHON} ${QUERY_SCRIPT} ${taskText.slice(0, 500)} --top 2 --min-score 0.35 --format json`.quiet()
        ...
        if (newPatterns.length > 0) {
          ...
          const contextLines = ["<uam-patterns>"]
          ...
          contextLines.push("</uam-patterns>")
          ...
          console.log(
            `[UAM-RAG] Injected ${newPatterns.length} pattern(s): ${newPatterns.map((p) => `P${p.id}`).join(", ")}`
          )
        }
      } catch {
