...
if grep -q '"agent":\s*"uap"' tools/agents/uap_agent.py; then
    echo "✅ Agent refactored from uap → uap"
else
    echo "⚠️  Note: Agent still named 'uap', but package name is correct for npm publish"
fi
