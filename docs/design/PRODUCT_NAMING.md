...
**Date:** 2026-07-08 — *availability data is a point-in-time sweep, see below*
**Status:** Analysis complete, no decision committed
**Landed:** 2026-08-20 (research preserved from an unmerged branch; no rename decided)

> **Re-check availability before acting on any candidate.** Every ✅ below is a
> registry/DNS observation from 2026-07-08, and this document's own central
> finding is that the space closes fast — two candidate spaces (`bosun`, `keel`)
> were claimed *during the cycle it was written in*. Treat the shortlist as
> still-valid reasoning and the ticks as expired evidence.
>
> What does NOT decay is the structural finding: the obvious supervision and
> herding metaphors are taken, several by products with this one's exact
> positioning, so a descriptive name is not available at any price. That is the
> part worth re-reading before the next naming discussion.
