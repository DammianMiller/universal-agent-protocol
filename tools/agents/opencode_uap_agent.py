...
7. When installing binaries, ALWAYS symlink to /usr/local/bin/ -- do NOT use /etc/profile.d/ (non-login shells won't see it).
8. For .pyx (Cython) files: apply the same fixes as .py files. Always rebuild extensions after editing .pyx files.
"""
...
def build_enhanced_instruction(instruction: str) -> str:
    """Build enhanced instruction with post-hook context prepended."""
...
ENV_BOOTSTRAP_CMD = (
    ...
    "echo \"Tools: $(which python3 gcc make sqlite3 curl git jq tesseract tesseract-ocr-eng file 2>/dev/null | tr '\\n' ' ')\"; "
    'echo "Files in /app/:"; ls -la /app/ 2>/dev/null | head -20; '
    'echo "Files in /app/tmp/:"; ls -la /app/tmp/ 2>/dev/null | head -20; '
    "echo \"OCR available: $(which tesseract 2>/dev/null && echo 'yes' || echo 'no')\"; "
    'echo "Image tools: $(which convert identify 2>/dev/null | wc -w) imagemagick tools"; '
    "echo '=== END BOOTSTRAP ==='"
)
...
class OpenCodeUAP(BaseInstalledAgent):
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        proxy_src = Path(__file__).parent / "scripts" / "tool-choice-proxy.js"
        if proxy_src.exists():
            await environment.upload_file(
                source_path=proxy_src,
                target_path="/installed-agent/tool-choice-proxy.js",
            )
            logger.info("[Layer 1] Uploaded tool-choice-proxy.js to container")
    ...
    def create_run_agent_commands(self, instruction: str) -> list[ExecInput]:
        ...
        proxy_cmd = (
            ...
            "nohup node /installed-agent/tool-choice-proxy.js > /tmp/proxy.log 2>&1 & "
            "PROXY_PID=$!; "
            "disown $PROXY_PID 2>/dev/null; "
            'echo "[Layer 1] Proxy PID: $PROXY_PID"; '
            # Wait for proxy to be ready
            "for i in $(seq 1 10); do "
            "  if curl -sf --max-time 1 http://127.0.0.1:11435/v1/models > /dev/null 2>&1; then "
            "    echo '[Layer 1] Proxy ready — forwarding to LLM with tool_choice=required'; "
            "    break; "
            "  fi; "
            "  sleep 0.5; "
            "done; "
            # Verify proxy is forwarding correctly
            "curl -sf --max-time 5 http://127.0.0.1:11435/v1/models > /dev/null 2>&1 "
            "&& echo '[Layer 1] Proxy -> LLM connectivity OK' "
            "|| echo '[Layer 1] WARNING: Proxy not forwarding to LLM'"
        )
