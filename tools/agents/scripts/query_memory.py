...
def query_long_term(query: str, limit=5):
    ...
    results = client.query_points(
        collection_name="claude_memory", query=query_embedding, limit=limit
    )
...
def store_long_term(type_: str, content: str, tags: list, importance: int):
    ...
    client.upsert(collection_name="claude_memory", points=[point])
...
def main():
    ...
    short_parser.add_argument(
        "-n", "--limit", type=int, default=50, help="Number of entries"
    )

    # Long-term query
    long_parser = subparsers.add_parser(
        "long", help="Semantic search in long-term memory"
    )
    long_parser.add_argument("query", help="Search query")
    long_parser.add_argument(
        "-n", "--limit", type=int, default=5, help="Number of results"
    )
    ...
    store_parser.add_argument(
        "type", choices=["fact", "skill", "preference", "lesson", "discovery"]
    )
