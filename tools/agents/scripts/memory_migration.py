...
class MemoryMigration:
                    ...
                    memories.append(
                        {
                            ...
                            "hash": hashlib.md5(
                                point.payload.get("content", "").encode()
                            ).hexdigest(),
                            "metadata": {
    ...
    def restore(self, backup_id: str):
        ...
        if sqlite_backup.exists():
            ...
            if self.short_term_db.exists():
                shutil.copy2(
                    self.short_term_db,
                    self.short_term_db.with_suffix(".db.pre_restore"),
                )
        ...
        if json_backup.exists():
            if self.long_term_json.exists():
                shutil.copy2(
                    self.long_term_json,
                    self.long_term_json.with_suffix(".json.pre_restore"),
                )
        ...
        if qdrant_backup.exists() and self.qdrant and self.model:
            ...
            if qdrant_data:
                # Recreate collection
                try:
                    self.qdrant.delete_collection("agent_memory")
                except Exception:
                    pass
    ...
    def list_backups(self):
        ...
        for b in backups:
            print(
                f"  {b['id']}  |  {b['created_at']}  |  SQLite: {b['sqlite_entries']}  Qdrant: {b['qdrant_entries']}"
            )
...
def main():
    ...
    export_parser.add_argument(
        "--format", choices=["native", "mem0", "amem"], default="native"
    )
