...
case "${1:-start}" in
    start)
        ...
        # Check if agent_memory collection exists
        COLLECTIONS=$(curl -s http://localhost:6333/collections | grep -o '"name":"[^"]*"' | grep agent_memory || true)
        if [ -z "$COLLECTIONS" ]; then
            echo "Creating agent_memory collection..."
            ...
            echo "agent_memory collection already exists"
        fi
    ...
    status)
        ...
        if curl -s http://localhost:6333/healthz > /dev/null 2>&1; then
            echo "Qdrant: RUNNING"
            POINTS=$(curl -s http://localhost:6333/collections/agent_memory 2>/dev/null | grep -o '"points_count":[0-9]*' | cut -d: -f2 || echo "0")
            echo "  - Collection: agent_memory ($POINTS points)"
        else
