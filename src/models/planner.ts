...
export class TaskPlanner {
  ...
  constructor(router: ModelRouter, config: MultiModelConfig, options: PlannerOptions = {}) {
    ...
    if (this.options.maxSubtasks !== undefined && this.options.maxSubtasks < 1) {
      throw new Error('maxSubtasks must be at least 1');
    }

    // Enforce maxDepth limit
    if (this.options.maxDepth !== undefined && this.options.maxDepth < 1) {
      throw new Error('maxDepth must be at least 1');
    }

    // Always create validator - validation runs on every plan at every level
    this.validator = createPlanValidator({
      enabled: true,
  ...
  /**
   * Create an execution plan for a task with automatic validation
   */
  async createPlan(taskDescription: string): Promise<ExecutionPlan> {
    const planId = uuidv4();
  ...
  private decomposeTask(taskDescription: string, complexity: TaskComplexity): Subtask[] {
    ...
    if (
      ...
      subtasks.push({
        ...
        description:
          'Create detailed design for the solution including interfaces, patterns, and structure',
        type: 'planning',
        complexity: 'high',
   ...
   */
  private estimateTotalCost(subtasks: Subtask[], assignments: Map<string, string>): number {
    let totalCost = 0;
   ...
   */
  private estimateDuration(subtasks: Subtask[], dependencies: Map<string, string[]>): number {
    ...
    while (queue.length > 0) {
      ...
      const maxDepTime =
        deps.length > 0 ? Math.max(...deps.map((d) => completionTimes.get(d) || 0)) : 0;
      ...
      for (const taskId of levelTasks) {
        ...
        if (subtask) {
          ...
          if (deps && deps.length > 0) {
            lines.push(
              `      Depends on: ${deps.map((d) => plan.subtasks.find((s) => s.id === d)?.title || d).join(', ')}`
            );
          }
        }
      }
