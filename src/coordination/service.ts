...
export class CoordinationService {
  ...
  claimResource(agentId: string, resource: string, claimType: ClaimType = 'exclusive'): boolean {
    ...
    // where two agents could simultaneously acquire "exclusive" claims
    const claimTransaction = this.db.transaction(() => {
      // Check for existing active claims on this resource
      const checkStmt = this.db.prepare(`
        SELECT agent_id, claim_type FROM work_claims
        WHERE resource = ? AND (expires_at IS NULL OR expires_at > ?)
      `);
      const existing = checkStmt.get(resource, now) as
        | { agent_id: string; claim_type: string }
        | undefined;
    ...
    try {
      return claimTransaction() as boolean;
    } catch {
      return false; // Constraint violation or transaction conflict
    }
  }
  ...
  detectOverlaps(resource: string, excludeAgentId?: string): WorkOverlap[] {
    ...
    const overlappingFiles = activeWork.filter((w) => {
      ...
      return w.filesAffected.some(
        (f) => f === resource || resource.includes(f) || f.includes(resource)
      );
    });
  ...
  private assessConflictRisk(work: WorkAnnouncement[], type: string = 'file'): ConflictRisk {
    ...
    const editors = work.filter(
      (w) => w.intentType === 'editing' || w.intentType === 'refactoring'
    );
  ...
  private generateOverlapSuggestion(work: WorkAnnouncement[], type: string): string {
    ...
    switch (type) {
      case 'same_file':
        return (
          `Multiple agents (${agentNames}) working on same file. Consider: ` +
      ...
      case 'same_directory':
        return (
          `Agents (${agentNames}) working in same directory. Usually safe with worktrees, ` +
          `but watch for: import changes, shared types/interfaces, barrel files (index.ts).`
        );

      case 'files_overlap':
        return (
          `Agents (${agentNames}) have overlapping file changes. Review affected files ` +
  ...
  private suggestMergeOrder(overlap: WorkOverlap): string[] {
    ...
    const priorityOrder: WorkIntentType[] = [
      'reviewing',
      'testing',
      'documenting',
      'editing',
  ...
  queueDeploy(
    ...
    const now = new Date().toISOString();
    const executeAfter =
      options.executeAfter?.toISOString() || new Date(Date.now() + 30000).toISOString(); // Default 30s delay for batching
