 ...
 */
export function calculateEffectiveImportance(entry: MemoryEntry, decayRate: number = 0.95): number {
...
export class HierarchicalMemoryManager {
  ...
  enforceTokenBudget(): number {
    ...
    while (hotTokens > this.config.hotMaxTokens && this.memory.hot.length > 1) {
      const lowest = this.memory.hot.reduce((min, e) =>
        ...
        calculateEffectiveImportance(min, this.config.decayRate)
          ? e
          : min
      );
      lowest.tier = 'warm';
  ...
  getStats(): {
    ...
    const coldTokens = this.memory.cold.reduce(
      (sum, e) => sum + estimateTokens(e.compressed || e.content),
      0
    );
