 ...
 * OPTIMIZATION 11: Added ambiguity detection (P37 pattern)
 */
export function classifyTask(instruction: string): TaskClassification {
  ...
  // OPTIMIZATION 11: Compute ambiguity score inline (lightweight version of P37)
  const ambiguityScore = computeQuickAmbiguity(instruction, bestCategory, confidence);
  ...
  return {
    ...
    memoryQueryHints: generateMemoryQueryHints(
      bestCategory,
      matchedKeywords.get(bestCategory) || []
    ),
...
/**
 * Quick ambiguity scoring for task classification (lightweight P37)
 * Full ambiguity detection is in ambiguity-detector.ts
 */
function computeQuickAmbiguity(
  instruction: string,
  category: TaskCategory,
  classificationConfidence: number
): { score: number; level: 'clear' | 'moderate' | 'high'; shouldAsk: boolean } {
  let score = 0;

  // Low classification confidence = higher ambiguity
  if (classificationConfidence < 0.2) score += 0.25;
  else if (classificationConfidence < 0.4) score += 0.1;

  // Unknown category = ambiguous
  if (category === 'unknown') score += 0.3;

  // Very short instructions
  const wordCount = instruction.split(/\s+/).length;
  if (wordCount <= 3) score += 0.3;
  else if (wordCount <= 6) score += 0.1;

  // Pronouns without clear antecedents
  if (
    /\b(it|that|this|the thing)\b/i.test(instruction) &&
    !/(\.ts|\.js|\.py|\.json|\.yaml|\.sh)/i.test(instruction)
  ) {
    score += 0.2;
  }

  // Vague action verbs without targets
  if (
    /\b(fix|optimize|improve|update|change)\b/i.test(instruction) &&
    !/(in|at|of|from|the\s+\w+\.(ts|js|py))/i.test(instruction)
  ) {
    score += 0.15;
  }

  // Missing success criteria
  if (!/\b(should|must|expect|verify|test|ensure|check)\b/i.test(instruction) && wordCount > 8) {
    score += 0.1;
  }

  score = Math.min(1.0, score);
  const level: 'clear' | 'moderate' | 'high' =
    score >= 0.6 ? 'high' : score >= 0.3 ? 'moderate' : 'clear';

  return { score, level, shouldAsk: level === 'high' };
}
