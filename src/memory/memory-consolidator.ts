...
export class MemoryConsolidator {
  ...
  private memoryQualityScores: Map<
    string,
    { score: number; lastAccessed: Date; accessCount: number }
  > = new Map();
  ...
  async consolidate(): Promise<ConsolidationResult> {
    ...
    const lessons = memories.filter(
      (m) => m.type === 'observation' && this.detectLesson(m.content)
    );
      ...
      for (const group of groups) {
        ...
        if (unit.compressionRatio > 1.2) {
          ...
          if (!this.contentHashes.has(hash)) {
            ...
            // Remove original summaries (parameterized to prevent SQL injection)
            const ids = group.map((s) => s.id);
            const placeholders = ids.map(() => '?').join(',');
            this.db
              .prepare(`DELETE FROM session_memories WHERE id IN (${placeholders})`)
              .run(...ids);
          }
        }
      }
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const decayed = Math.round(
          row.importance * Math.pow(this.config.qualityDecayRate, daysSince)
        );
