...
export class QdrantCloudBackend implements MemoryBackend {
  ...
  private async ensureCollection(): Promise<void> {
    ...
    const currentSize = (collectionInfo.config?.params as { vectors?: { size?: number } })?.vectors
      ?.size;
    ...
    if (currentSize && currentSize !== this.vectorSize) {
      ...
      if (!newExists) {
        ...
        console.log(
          `[Qdrant] Created new collection ${newCollectionName} with ${this.vectorSize} dimensions`
        );
      }
