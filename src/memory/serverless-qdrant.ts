...
export class ServerlessQdrantManager {
      ...
      try {
        ...
        spawn(
          ...
          [
            ...
            `${port}:6333`,
            '-v',
            `${process.cwd()}/${dataDir}:/qdrant/storage`,
            '-d',
            dockerImage,
          ],
  ...
  async search(
    ...
    if (backend === 'memory') {
      ...
      return this.inMemorySearch(
        vectors as Array<{ id: string; vector: number[]; payload: Record<string, unknown> }>,
        vector,
        limit
      );
    }
  ...
  getStatus(): {
    ...
    return {
      ...
      inMemorySize: Array.from(this.inMemoryFallback.values()).reduce(
...
export function getServerlessQdrantManager(
  config?: QdrantServerlessConfig
