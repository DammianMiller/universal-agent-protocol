...
export async function prepopulateMemory(
  ...
  if (doAll || options.docs) {
    ...
    shortTermMemories.push(
      ...docMemories.filter((m) => m.importance && m.importance >= 7).slice(0, 20)
    );
  ...
  if (doAll || options.git) {
    ...
    shortTermMemories.push(
      ...gitMemories.filter((m) => m.importance && m.importance >= 6).slice(0, 30)
    );
  ...
  if (doAll || options.skills) {
    ...
    shortTermMemories.push(
      ...skillMemories.filter((m) => m.importance && m.importance >= 7).slice(0, 10)
    );
  }
      ...
      if (/^[0-9a-f]+$/i.test(trimmedHash)) {
        try {
          const filesOutput = execFileSync(
            'git',
            ['diff-tree', '--no-commit-id', '--name-only', '-r', trimmedHash],
            {
              cwd,
              encoding: 'utf-8',
              stdio: 'pipe',
...
function calculateDocImportance(section: ParsedDocument['sections'][0], filePath: string): number {
  let importance = 5;
...
function calculateCommitImportance(commit: GitCommit): number {
  ...
  const coreFiles = commit.files.filter(
    (f) =>
      f.includes('config') ||
...
export function extractSkillMemories(skills: DiscoveredSkill[]): MemoryEntry[] {
  ...
  const usefulArtifacts = artifacts.filter(
    (a) => a.path.includes('workflow') || a.path.includes('template') || a.path.includes('script')
  );
