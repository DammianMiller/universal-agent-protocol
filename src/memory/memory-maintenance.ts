...
function applyDecay(db: Database.Database, cfg: MaintenanceConfig): number {
  ...
  try {
    ...
    `
      ...
      .all(cfg.minImportanceAfterDecay) as Array<{
      id: number;
      importance: number;
      timestamp: string;
    }>;
...
function generateRecommendations(
  ...
  try {
    ...
    if (memCount > 45) {
      recs.push(
        `Working memory near capacity (${memCount}/50). Consider promoting important entries and pruning old ones.`
      );
    }
    ...
    `
        ...
        .get(staleDate.toISOString()) as { c: number }
    ).c;

    if (staleCount > 0) {
      recs.push(
        `${staleCount} stale entries with low importance found. Run maintenance again or review manually.`
      );
    }
  } catch {
    // Table might not exist
  ...
  try {
    ...
    if (unpromoted > 10) {
      recs.push(
        `${unpromoted} unpromoted daily log entries. Run \`uap memory promote\` to review.`
      );
    }
  } catch {
    // Table might not exist
  ...
  try {
    ...
    if (lowImp > 20) {
      recs.push(
        `${lowImp} session memories with very low importance. These may be pruned on next maintenance.`
      );
    }
  } catch {
    // Table might not exist
  ...
  if (staleWorktrees.length > 0) {
    recs.push(
      `${staleWorktrees.length} stale worktrees found (>${cfg.archiveDaysOld} days old): ${staleWorktrees.slice(0, 5).join(', ')}${staleWorktrees.length > 5 ? '...' : ''}. Run \`uap worktree cleanup <id>\` to remove.`
    );
  }
  ...
  try {
    ...
    try {
      summary.dailyLogCount = (
        db.prepare('SELECT COUNT(*) as c FROM daily_log').get() as { c: number }
      ).c;
      summary.unpromotedCount = (
        db.prepare('SELECT COUNT(*) as c FROM daily_log WHERE promoted = 0').get() as { c: number }
      ).c;
    } catch {
      /* table doesn't exist */
    }
    ...
    summary.healthy =
      summary.memoriesCount <= 45 && summary.staleCount < 10 && summary.unpromotedCount < 20;
  } finally {
    db.close();
  }
