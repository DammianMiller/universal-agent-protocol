 ...
 */
export function compressMemoryEntry(
  content: string,
...
/**
 * Compression tier based on remaining budget
 */
export type CompressionTier = 'light' | 'medium' | 'aggressive';
...
export class DynamicCompressor {
   ...
   */
  private determineTier(budgetRemaining: number, budgetTotal: number): CompressionTier {
    if (budgetTotal <= 0) return 'aggressive';
   ...
   */
  private compressLight(content: string): string {
    let result = content;
   ...
   */
  private compressMedium(content: string): string {
    ...
    const kept = sentences.filter((s) => {
      ...
      if (/^(let me|i will|i'll|okay|alright|sure|thanks|thank you)/i.test(lower)) return false;
      if (/^(as mentioned|as noted|as stated|as discussed)/i.test(lower)) return false;
      if (/^(in conclusion|to summarize|in summary|overall)/i.test(lower)) return false;
      return true;
    });
   ...
   */
  private compressAggressive(content: string): string {
    ...
    const scored = sentences.map((s) => {
      ...
      if (/`[^`]+`|"[^"]+"|'[^']+'/.test(s)) score += 2;
      if (/\.(ts|js|py|sh|json|yaml|md)\b|\/[\w/]+/.test(s)) score += 2;

      // Boost for actionable/important language
      if (/^(use|run|execute|call|apply|implement|add|remove|fix|change)/i.test(s)) score += 2;
      if (/careful|watch out|gotcha|avoid|don't|never|must|critical|important/i.test(s)) score += 3;
      if (/decided|chose|selected|learned|realized|discovered/i.test(s)) score += 2;

      // Penalty for vague language
      if (/might|maybe|perhaps|possibly|somewhat|kind of/i.test(s)) score -= 1;
      if (/I think|I believe|in my opinion|it seems/i.test(s)) score -= 1;
