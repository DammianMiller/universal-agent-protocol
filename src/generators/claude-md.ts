...
async function buildContext(
  ...
  return {
    ...
    DATABASE_ARCHITECTURE:
      analysis.databases.length > 0 ? buildDatabaseArchitecture(analysis) : null,
    ...
    // Infrastructure
    HAS_INFRA:
      analysis.directories.infrastructure.length > 0 || config.template?.sections?.pipelineOnly,
    ...
    IS_BENCHMARK:
      config.template?.sections?.benchmark ||
    ...
    HAS_PROJECT_MD:
      existsSync(join(cwd, '.factory/PROJECT.md')) ||
    ...
    if (existsSync(fullPath) && statSync(fullPath).isDirectory()) {
      ...
      try {
        ...
        for (let i = 0; i < subdirs.length; i++) {
          ...
          const subComment = getSubdirComment(
            path,
            subdirs[i].name,
            join(fullPath, subdirs[i].name)
          );
          lines.push(
            `${prefix}${subdirs[i].name}/`.padEnd(35) + (subComment ? `# ${subComment}` : '')
          );
        }
      } catch {
...
function buildLanguageDroidsTable(droids: DiscoveredSkill[], languages: string[]): string | null {
  const languageDroids = droids.filter(
    (d) =>
      d.name.includes('-pro') ||
...
function buildTroubleshootingSection(
  memories: Array<{ content: string; tags?: string[]; metadata?: Record<string, unknown> }>
): string | null {
...
function buildArchitectureOverview(analysis: ProjectAnalysis): string | null {
  ...
  if (analysis.components.length > 0) {
    ...
    for (const comp of analysis.components.slice(0, 8)) {
      sections.push(
        `- **${comp.name}** (\`${comp.path}\`): ${comp.description || `${comp.language} ${comp.framework || 'application'}`}`
      );
    }
    sections.push('');
  }
...
function buildPrimarySkills(skills: DiscoveredSkill[]): string | null {
  const primary = skills
    .filter(
      (s) => s.name.includes('design') || s.name.includes('frontend') || s.name.includes('ui')
    )
    .slice(0, 3);
  ...
  return primary
    .map(
      (s) => `│     ├─ Use ${s.name} for ${s.description || 'specialized work'}                  │`
    )
    .join('\n');
}
...
function buildFileTypeRouting(languages: string[]): string | null {
  ...
  for (const lang of languages) {
    ...
    if (
      (langLower.includes('typescript') || langLower.includes('javascript')) &&
      !added.has('ts')
    ) {
      routing.push(
...
function buildRelevantPatterns(analysis: ProjectAnalysis): string | null {
  ...
  const hasSecurity = languages.some(
    (l) => l.includes('python') || l.includes('javascript') || l.includes('typescript')
  );
  const hasGit = true; // All projects use git
  const hasCLI = analysis.components.some(
    (c) =>
  ...
  const high: string[] = ['P3 (StateProtect)', 'P17 (CE)', 'P32 (CEV)'];
  ...
  if (
    analysis.components.some(
      (c) =>
        c.name.toLowerCase().includes('chess') || c.description?.toLowerCase().includes('chess')
    )
  ) {
    domain.push('P21 (CEI)');
  }
  if (
    analysis.components.some(
      (c) =>
        c.name.toLowerCase().includes('compress') ||
...
function isNoisyContent(content: string): boolean {
  ...
  if (content.startsWith('$ uap') || content.startsWith('$uap')) return true;
  ...
  if (
    content.includes('never commits directly to main') ||
...
function getDesktopTemplate(): string {
  ...
  const nodeModulesPath = join(
    process.cwd(),
    'node_modules/universal-agent-protocol/templates/CLAUDE.template.md'
  );
  ...
  console.info(
    'Using built-in fallback template. For full features, ensure templates/CLAUDE.template.md exists.'
  );
