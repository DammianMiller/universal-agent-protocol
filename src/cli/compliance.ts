...
import { ensureShortTermSchema, ensureSessionSchema, ensureKnowledgeSchema, initializeMemoryDatabase } from '../memory/short-term/schema.js';
...
export async function complianceCommand(action: ComplianceAction, options: ComplianceOptions = {}): Promise<void> {
...
async function runComplianceCheck(cwd: string, options: ComplianceOptions = {}): Promise<void> {
  ...
  if (existsSync(dbPath)) {
    ...
    results.push({ name: 'Memory database', status: 'fail', message: 'Database not found', fixable: true });
  }
    ...
    try {
      ...
      for (const table of ['memories', 'session_memories', 'entities', 'relationships']) {
        if (tables.includes(table)) {
          ...
          results.push({ name: `Table '${table}'`, status: 'fail', message: 'Missing', fixable: true });
        }
      }
      ...
      const entityCols = (db.prepare("PRAGMA table_info(entities)").all() as Array<{ name: string }>).map(c => c.name);
      if (entityCols.includes('description')) {
        results.push({ name: 'entities.description', status: 'pass', message: 'Column exists' });
      } else {
        results.push({ name: 'entities.description', status: 'warn', message: 'Missing column (run uap compliance fix)', fixable: true });
      }

      // Gate 5: Schema compliance — relationships has strength column
      const relCols = (db.prepare("PRAGMA table_info(relationships)").all() as Array<{ name: string }>).map(c => c.name);
      if (relCols.includes('strength')) {
        results.push({ name: 'relationships.strength', status: 'pass', message: 'Column exists' });
      } else {
        results.push({ name: 'relationships.strength', status: 'warn', message: 'Missing column (run uap compliance fix)', fixable: true });
      }
      ...
      try {
        ...
        try {
          dbRW.prepare("INSERT INTO memories (timestamp, type, content) VALUES ('__test__', 'lesson', '__test__')").run();
          dbRW.exec("DELETE FROM memories WHERE content = '__test__' AND timestamp = '__test__'");
          results.push({ name: 'memories.type (lesson)', status: 'pass', message: 'Lesson type allowed' });
        } catch {
          results.push({ name: 'memories.type (lesson)', status: 'warn', message: 'Lesson type blocked by old CHECK constraint', fixable: true });
        }
        dbRW.close();
      } catch {
        results.push({ name: 'memories.type (lesson)', status: 'warn', message: 'Could not verify type constraint' });
      }

      // Gate 7: Recent memory activity
      try {
        const dbCheck = new Database(dbPath, { readonly: true });
        const recent = dbCheck.prepare("SELECT COUNT(*) as cnt FROM memories WHERE timestamp >= datetime('now', '-2 hours')").get() as { cnt: number };
        if (recent.cnt > 0) {
          results.push({ name: 'Recent activity', status: 'pass', message: `${recent.cnt} entries in last 2 hours` });
        } else {
          results.push({ name: 'Recent activity', status: 'warn', message: 'No entries in last 2 hours' });
        }

        // Gate 8: Session memories exist
        const sessionCount = dbCheck.prepare("SELECT COUNT(*) as cnt FROM session_memories").get() as { cnt: number };
        if (sessionCount.cnt > 0) {
          results.push({ name: 'Session memories', status: 'pass', message: `${sessionCount.cnt} session memories stored` });
        } else {
          results.push({ name: 'Session memories', status: 'warn', message: 'No session memories stored yet' });
        }
      ...
      results.push({ name: 'Database access', status: 'fail', message: `Cannot open database: ${err}` });
    }
  }
  ...
  if (existsSync(coordPath)) {
    ...
    results.push({ name: 'Coordination DB', status: 'warn', message: 'Not found (single-agent mode)' });
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (collections.includes(requiredCollection) || collections.includes('uam_memory')) {
        results.push({ name: 'Qdrant L3 collection', status: 'pass', message: `Collection found` });
      } else {
        results.push({ name: 'Qdrant L3 collection', status: 'warn', message: `Collection '${requiredCollection}' not found`, fixable: true });
      }
      ...
      if (collections.includes(patternCollection)) {
        ...
        try {
          const colRes = await fetch(`${url}/collections/${patternCollection}`, { signal: AbortSignal.timeout(2000) });
          if (colRes.ok) {
            const colData = await colRes.json() as { result: { points_count: number } };
            if (colData.result.points_count > 0) {
              results.push({ name: 'Pattern RAG collection', status: 'pass', message: `${colData.result.points_count} patterns indexed` });
            } else {
              results.push({ name: 'Pattern RAG collection', status: 'warn', message: 'Collection exists but is empty (run uap patterns index)', fixable: true });
            }
          }
        } catch {
          results.push({ name: 'Pattern RAG collection', status: 'warn', message: 'Could not check collection' });
        }
      } else {
        results.push({ name: 'Pattern RAG collection', status: 'warn', message: `Collection '${patternCollection}' not found` });
      }
    } else {
      results.push({ name: 'Qdrant', status: 'warn', message: 'Not responding' });
    }
  } catch {
    results.push({ name: 'Qdrant', status: 'warn', message: 'Not reachable (optional for L3/Pattern RAG)' });
  }
  ...
  try {
    ...
    if (branch === 'main' || branch === 'master') {
      results.push({ name: 'Worktree compliance', status: 'warn', message: `On ${branch} branch — use a feature branch for changes` });
    } else {
      results.push({ name: 'Worktree compliance', status: 'pass', message: `On feature branch: ${branch}` });
    }

    // Check for stale worktrees
    const worktreeOutput = execSync('git worktree list --porcelain', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
    const prunable = (worktreeOutput.match(/prunable/g) || []).length;
    if (prunable > 0) {
      results.push({ name: 'Stale worktrees', status: 'warn', message: `${prunable} prunable worktrees (run git worktree prune)`, fixable: true });
    } else {
  ...
  try {
    ...
    if (staged) {
      const secretPatterns = execSync('git diff --cached | grep -iE "(password|secret|api_key|token|private_key)" || true', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
      if (secretPatterns) {
        results.push({ name: 'Secret detection', status: 'fail', message: 'Potential secrets in staged files!' });
      } else {
  ...
  console.log('');
  console.log(chalk.bold('========================================'));
  console.log(chalk.bold(`  COMPLIANCE: ${passes}/${results.length} passed, ${warns} warnings, ${fails} failures`));
  console.log(chalk.bold('========================================'));
...
async function runComplianceFix(cwd: string, _options: ComplianceOptions): Promise<void> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Create uam_memory if missing
      const ltCollection = config?.memory?.longTerm?.collection || 'agent_memory';
      if (!existing.includes(ltCollection) && !existing.includes('uam_memory')) {
