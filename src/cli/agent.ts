...
import { AutoAgentCoordinator } from '../coordination/auto-agent.js';
...
export async function agentCommand(action: AgentAction, options: AgentOptions = {}): Promise<void> {
  ...
  switch (action) {
    case 'register':
      ...
      break;
    case 'auto':
      await autoRegisterAgent(service, options);
      break;
    case 'heartbeat':
...
async function registerAgent(service: CoordinationService, options: AgentOptions): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.dim(
        '  1. Announce your work: uap agent announce --id <id> --resource <file> --intent editing'
      )
    );
...
async function showAgentStatus(service: CoordinationService, options: AgentOptions): Promise<void> {
  ...
  if (id) {
    ...
    if (agent.capabilities && agent.capabilities.length > 0) {
      for (const line of keyValue([['Capabilities', agent.capabilities.join(', ')]]))
        console.log(line);
    }
    ...
    if (agents.length === 0) {
      ...
      for (const agent of agents) {
        console.log(
          `  ${statusBadge(agent.status)} ${chalk.cyan(chalk.bold(agent.name))} ${chalk.dim(`(${agent.id.slice(0, 8)}...)`)}`
        );
        if (agent.currentTask) {
...
async function announceWork(service: CoordinationService, options: AgentOptions): Promise<void> {
  ...
  const validIntents: WorkIntentType[] = [
    'editing',
    'reviewing',
    'refactoring',
    'testing',
    ...
    if (overlaps.length === 0) {
      ...
      for (const overlap of overlaps) {
        ...
        for (const agent of overlap.agents) {
          console.log(
            chalk.dim(`    - ${agent.name || agent.id.slice(0, 8)} (${agent.intentType})`)
          );
      ...
      if (suggestions.length > 0) {
        ...
        for (const suggestion of suggestions) {
          ...
          if (suggestion.suggestedOrder) {
            console.log(
              chalk.dim(`      Suggested merge order: ${suggestion.suggestedOrder.join(' → ')}`)
            );
          }
        }
      }
  ...
  if (!resource) {
    ...
    for (const [res, works] of grouped) {
      ...
      for (const work of works) {
        console.log(
          `    ${chalk.cyan(work.agentName || work.agentId.slice(0, 8))} ${chalk.dim(work.intentType)}`
        );
    ...
    for (const overlap of overlaps) {
      ...
      for (const agent of overlap.agents) {
        console.log(
          `    ${chalk.cyan(agent.name || agent.id.slice(0, 8))} ${chalk.dim(`(${agent.intentType})`)}`
        );
        if (agent.worktreeBranch) {
          console.log(chalk.dim(`      Branch: ${agent.worktreeBranch}`));
        }
...
async function autoRegisterAgent(
  ...
  const name = options.name || `auto-agent-${Date.now()}`;
  const capabilities = options.capabilities?.split(',').map((c) => c.trim());
  const worktree = options.worktree;

  const spinner = ora(`Auto-registering agent: ${name}...`).start();

  try {
    const autoAgent = new AutoAgentCoordinator({
      name,
      capabilities,
    ...
    spinner.succeed(`Agent auto-registered: ${name}`);
    console.log(chalk.bold.green(`AGENT_ID=${result.agent.id}`));
    console.log(chalk.dim('Use this ID for subsequent agent commands'));
    console.log(chalk.dim(`Heartbeat interval: 30s (auto-cleanup on exit)`));
    console.log('');
    console.log(chalk.cyan('Next steps:'));
    console.log(
      chalk.dim(
    ...
    console.log(chalk.dim('  2. Check for overlaps: uap agent overlaps --resource <file>'));
    console.log(chalk.dim('  3. When done: uap agent complete --id <id> --resource <file>'));
    console.log(chalk.dim('  4. Agent will auto-deregister on process exit'));
  } catch (error) {
    spinner.fail('Failed to auto-register agent');
    console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    process.exit(1);
  }
}
