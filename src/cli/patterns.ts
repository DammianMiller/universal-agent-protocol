...
function getPatternRagConfig(config: AgentContextConfig) {
  return {
    ...
    indexScript:
      config.memory?.patternRag?.indexScript ?? './agents/scripts/index_patterns_to_qdrant.py',
...
async function showPatternStatus(cwd: string): Promise<void> {
  ...
  console.log(
    `  ${chalk.dim('Enabled:')}       ${rag.enabled ? chalk.green('yes') : chalk.yellow('no')}`
  );
  ...
  console.log('');
  console.log(
    `  ${chalk.dim('Python:')}       ${python ? chalk.green(python) : chalk.red('not found')}`
  );
  console.log(
    `  ${chalk.dim('Index script:')} ${existsSync(join(cwd, rag.indexScript)) ? chalk.green('found') : chalk.yellow('missing')}`
  );
  console.log(
    `  ${chalk.dim('Query script:')} ${existsSync(join(cwd, rag.queryScript)) ? chalk.green('found') : chalk.yellow('missing')}`
  );
  console.log('');
}
...
async function indexPatterns(cwd: string, options: PatternOptions): Promise<void> {
  ...
  if (existsSync(scriptPath)) {
    ...
    if (!python) {
      ...
      console.log(
        chalk.dim(
          '  Or create venv: python3 -m venv agents/.venv && agents/.venv/bin/pip install sentence-transformers qdrant-client'
        )
      );
      return;
    }
...
async function queryPatterns(cwd: string, options: PatternOptions): Promise<void> {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        for (const p of patterns) {
          ...
          console.log(
            `  ${chalk.dim(p.body.slice(0, rag.maxBodyChars))}${p.body.length > rag.maxBodyChars ? '...' : ''}`
          );
        }
      } else {
        console.log(output);
  ...
  console.log(
    chalk.yellow('  Query script not found. Using fallback keyword search (less accurate).')
  );
  console.log(
    chalk.dim(
      `  For semantic search, run \`uap patterns generate\` to create the Python scripts.\n`
    )
  );
  ...
  try {
    ...
    for (const m of matches) {
      ...
      console.log(
        `  ${chalk.dim(body.slice(0, rag.maxBodyChars))}${body.length > rag.maxBodyChars ? '...' : ''}`
      );
    }
    console.log('');
  } catch {
