...
export class TaskService {
  ...
  create(input: CreateTaskInput): Task {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO tasks (id, title, description, type, status, priority, assignee, labels, notes, parent_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
  ...
  getWithRelations(id: string): TaskWithRelations | null {
    ...
    const blockedBy = deps.filter(d => d.fromTask === id && d.depType === 'blocks').map(d => d.toTask);
    const blocks = deps.filter(d => d.toTask === id && d.depType === 'blocks').map(d => d.fromTask);
    const relatedTo = deps.filter(d => d.depType === 'related').map(d => d.fromTask === id ? d.toTask : d.fromTask);
  ...
  update(id: string, input: UpdateTaskInput): Task | null {
    ...
    if (input.description !== undefined && input.description !== existing.description) {
      ...
      changes.push({ field: 'description', oldValue: existing.description || null, newValue: input.description || null });
    }
    ...
    if (input.priority !== undefined && input.priority !== existing.priority) {
      ...
      changes.push({ field: 'priority', oldValue: String(existing.priority), newValue: String(input.priority) });
    }

    if (input.assignee !== undefined && input.assignee !== existing.assignee) {
      updates.push('assignee = ?');
      params.push(input.assignee || null);
      changes.push({ field: 'assignee', oldValue: existing.assignee || null, newValue: input.assignee || null });
    }
    ...
    if (input.worktreeBranch !== undefined && input.worktreeBranch !== existing.worktreeBranch) {
      ...
      changes.push({ field: 'worktreeBranch', oldValue: existing.worktreeBranch || null, newValue: input.worktreeBranch || null });
    }
    ...
    if (input.notes !== undefined && input.notes !== existing.notes) {
      ...
      changes.push({ field: 'notes', oldValue: existing.notes || null, newValue: input.notes || null });
    }
  ...
  addDependency(fromTask: string, toTask: string, depType: DependencyType = 'blocks'): TaskDependency | null {
  ...
  private recordHistory(taskId: string, field: string, oldValue: string | null, newValue: string | null): void {
    const stmt = this.db.prepare(`
  ...
  getStats(): TaskStats {
    ...
    const byStatus: Record<TaskStatus, number> = {
      open: 0, in_progress: 0, blocked: 0, done: 0, wont_do: 0
    };
    const statusRows = this.db.prepare('SELECT status, COUNT(*) as count FROM tasks GROUP BY status').all() as Array<{ status: TaskStatus; count: number }>;
    for (const row of statusRows) {
      byStatus[row.status] = row.count;
    }

    const byType: Record<TaskType, number> = {
      task: 0, bug: 0, feature: 0, epic: 0, chore: 0, story: 0
    };
    const typeRows = this.db.prepare('SELECT type, COUNT(*) as count FROM tasks GROUP BY type').all() as Array<{ type: TaskType; count: number }>;
    for (const row of typeRows) {
      byType[row.type] = row.count;
    }

    const byPriority: Record<TaskPriority, number> = { 0: 0, 1: 0, 2: 0, 3: 0, 4: 0 };
    const priorityRows = this.db.prepare('SELECT priority, COUNT(*) as count FROM tasks GROUP BY priority').all() as Array<{ priority: TaskPriority; count: number }>;
    ...
    return {
      ...
      blocked,
      ready,
      overdue: 0, // TODO: implement with due dates
    };
  }
        ...
        if (existing) {
          ...
          if (data.updatedAt > existing.updatedAt) {
            this.db.prepare(`
              ...
              WHERE id = ?
            `).run(
              data.title, data.description || null, data.type, data.status, data.priority,
              data.assignee || null, data.worktreeBranch || null, JSON.stringify(data.labels),
              data.notes || null, data.parentId || null, data.updatedAt,
              data.closedAt || null, data.closedReason || null, data.id
            );
          }
        } else {
          // Insert new task
          this.db.prepare(`
            INSERT INTO tasks (id, title, description, type, status, priority, assignee,
              worktree_branch, labels, notes, parent_id, created_at, updated_at, closed_at, closed_reason)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(
            data.id, data.title, data.description || null, data.type, data.status, data.priority,
            data.assignee || null, data.worktreeBranch || null, JSON.stringify(data.labels),
            data.notes || null, data.parentId || null, data.createdAt, data.updatedAt,
            data.closedAt || null, data.closedReason || null
          );
          imported++;
        }
  ...
  compact(olderThanDays = 90): TaskSummary | null {
    ...
    `);
    const oldTasks = stmt.all(cutoff) as Array<{ id: string; title: string; type: string; labels: string; closedAt: string }>;
    ...
    for (const [period, tasks] of quarters) {
      ...
      const summary = `Completed ${tasks.length} tasks: ${tasks.slice(0, 5).map(t => t.title).join(', ')}${tasks.length > 5 ? '...' : ''}`;
      ...
      this.db.prepare(`DELETE FROM tasks WHERE id IN (${ids.map(() => '?').join(',')})`).run(...ids);
    }
