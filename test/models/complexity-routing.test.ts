...
describe('complexity-tier routing presets', () => {
  it('ships cost-tiered and speed-tiered presets with a full tier map', () => {
    ...
    expect(cost.tiers).toMatchObject({ low: 'qwen36-a3b', high: 'opus-4.8', critical: 'opus-4.8' });
  ...
  it('tiersToRoutingMatrix materializes a single-model-per-tier map (router form)', () => {
    ...
    expect(m).toEqual({ low: 'qwen36-a3b', medium: 'qwen36-a3b', high: 'opus-4.8', critical: 'opus-4.8' });
  ...
  it('resolvePresetModel: complexity tier wins for execution, role otherwise', () => {
    ...
    expect(resolvePresetModel(cost, { complexity: 'low' })).toBe('qwen36-a3b');
    ...
    expect(resolvePresetModel(cost, {})).toBe('qwen36-a3b');
