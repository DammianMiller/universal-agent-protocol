...
| ------------------ | -------------- | -------------------------------------------------------------------------------- |
| Memory             | 27 modules     | 4-layer persistent memory (Working, Session, Semantic, Knowledge) with Qdrant vectors |
| Coordination       | 8 modules      | Multi-agent lifecycle, work claims, overlap detection, and Pattern routing           |
| Deploy Batching    | 1 module       | Squash, merge, and parallelize deploy actions across agents                          |
| Policy Enforcement | 8 modules      | Middleware-based enforcement of operational gates with Python-based validators      |
| Browser            | 1 module       | Stealth web automation via CloakBrowser (Playwright drop-in)                     |
| MCP Router         | 10 modules     | Meta-router for tools with integrated Output Compression (98% token savings)       |
| Models             | 10 modules     | Multi-model routing, planning, and execution with 13 model profiles                |
| Patterns           | 23 patterns    | Battle-tested workflows (P-codes) from Terminal-Bench 2.0                            |
| Droids             | 8 experts      | Specialized agents with strict validation pipelines (JSON Schema, Decoder-First)    |
| Skills             | 33 skills      | Reusable domain expertise (chess, polyglot, compression, etc.)                   |
| Tasks              | 7 modules      | Full task lifecycle with dependencies, claims, and JSONL sync                      |
...
| Pattern Router        | \`src/coordination/pattern-router.ts\`    | Matches tasks to P-code patterns from \`.factory/patterns/\` |
...
| Output Compressor | \`src/mcp-router/output-compressor.ts\` | Strips redundant data from tool responses    |
