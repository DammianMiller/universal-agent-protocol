...
- Document your reasoning process`
  ...
  return genericContexts[category] || '## General Best Practices\n- Follow standard development practices';
}
...
async function runTask(task: typeof TESTS[0]) {
  ...
  const messages = [
    ...
    { role: 'user', content: `You need to solve a software development task in the ${task.category} domain. Provide your solution with clear steps.` }
  ];
...
async function main() {
  ...
  const prevResults = JSON.parse(readFileSync('/home/cogtek/dev/miller-tech/universal-agent-memory/benchmark-results/qwen35_direct_uap/qwen35_direct_uap_2026-03-11T01-54-38.json', 'utf8'));
  console.log(`\nComparison with UAP benchmark (with task-specific patterns):`);
  console.log(`  Previous success rate: ${prevResults.successRate.toFixed(1)}% (${Math.round(prevResults.totalTasks)} tasks)`);
  console.log(`  Generic UAP success rate: ${successRate.toFixed(1)}% (${total} tasks)`);

  mkdirSync(RESULTS_DIR, { recursive: true });
  writeFileSync(join(RESULTS_DIR, `results_${TIMESTAMP}.json`), JSON.stringify({ 
    timestamp: TIMESTAMP, 
