...
skills_to_update = {
    ...
    "codebase-navigator": ("Navigation", ["navigate", "codebase", "explore", "understand"]),
    ...
    "terminal-bench-strategies": ("Benchmarking", ["terminal-bench", "benchmark", "test", "evaluate"]),
}
...
"""
    if content.startswith("---"):
        ...
        if end_frontmatter > 0:
            return content[:end_frontmatter+3] + rtk_addition + content[end_frontmatter+3:]
...
"""
    if "## Common Pitfalls" in content:
        return content.replace("## Common Pitfalls", uam_section + "\n## Common Pitfalls")
